...
class ResManager {
    ...
    private _sysMgrCache: Map<string, CacheData> = new Map();

    /** 框架增加addRef 使用者自己负责decRef*/
    private _customMgrCache: Map<string, CacheData> = new Map();
    ...
    private cacheAsset(url: string, asset: cc.Asset, isAddRef: boolean = true, release: boolean = true): void {
        ...
        if (asset instanceof cc.Prefab) {
            ...
            func(this._sysMgrCache);
        }
        else {
            func(this._customMgrCache);
        }
    }
    ...
    */
     ...
     * @param url 
     */
    public releaseCustomMgrCacheByUrl(url: string) {
        let data = this._customMgrCache.get(url);
        if (data) {
            data.asset.decRef();
            this._customMgrCache.delete(url);
        }
    }
     ...
     * 释放所有自定义管理的资源缓存
     */
    public releaseAllCustomMgrCache() {
        let arr = this._customMgrCache;
        arr.forEach((cacheData, url) => {
